...
/**
 * RouteClickListener
 * is used to edit routes via Mouse interaction
 * 
 */
public class RouteClickListener extends MouseAdapter {
    ...
    public void mapClicked(GeoPosition location) {
    	RouteManagerController.INSTANCE.waypointTableView.getItems().add(new WaypointModel(""+(RouteManagerController.INSTANCE.waypointTableView.getItems().size()+1), location.getLatitude(), location.getLongitude()));
